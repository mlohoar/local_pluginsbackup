...
function local_pluginsbackup_recursiveDelete($str, bool $delete_dir = false): string
{
    ...
    local_pluginsbackup_deleteContents($str);

    return '';
}

function local_pluginsbackup_deleteContents($dir)
{
    if (!is_dir($dir)) {
    ...
    foreach ($files as $file) {
        $filePath = "$dir/$file";
        if (is_dir($filePath)) {
            local_pluginsbackup_deleteContents($filePath);
            rmdir($filePath); // Remove empty directory
        } else {
